...
<script>
import { addClass, removeClass } from '@app/oda-component/utils/dom';
import PlusIcon from '@app/oda-component/icons/plus';
import TimesIcon from '@app/oda-component/icons/times';
import UploadIcon from '@app/oda-component/icons/upload';
import Button from '@app/oda-component/button';
import Message from '@app/oda-component/message';
import ProgressBar from '@app/oda-component/progressbar';
...
export default {
    ...
    methods: {
        ...
        formatSize(bytes) {
            const k = 1024;
            const dm = 3;
            const sizes = this.$OdaComponents.config.locale?.fileSizeTypes || ['B', 'KB', 'MB', 'GB', 'TB', 'PB', 'EB', 'ZB', 'YB'];
    ...
    computed: {
        ...
        basicFileChosenLabel() {
            if (this.auto) return this.chooseButtonLabel;
                ...
                return this.$OdaComponents.config.locale?.fileChosenMessage?.replace('{0}', this.files.length);
            }

            return this.$OdaComponents.config.locale?.noFileChosenMessage || '';
        },
        hasFiles() {
        ...
        chooseButtonLabel() {
            return this.chooseLabel || this.$OdaComponents.config.locale.choose;
        },
        uploadButtonLabel() {
            return this.uploadLabel || this.$OdaComponents.config.locale.upload;
        },
        cancelButtonLabel() {
            return this.cancelLabel || this.$OdaComponents.config.locale.cancel;
        },
        completedLabel() {
            return this.$OdaComponents.config.locale.completed;
        },
        pendingLabel() {
            return this.$OdaComponents.config.locale.pending;
        }
    },
    components: {
        Button,
