...
function resolveLegacyPrimeuixPackage(source: string): string | null {
    if (source === '@primeuix/utils' || source.startsWith('@primeuix/utils/')) {
        const mapped = source.replace('@primeuix/utils', '@oda-components/utils');

        return resolveDistPackage(mapped);
    }

    if (source === '@primeuix/styled' || source.startsWith('@primeuix/styled/')) {
        const mapped = source.replace('@primeuix/styled', '@oda-components/styled');

        return resolveDistPackage(mapped);
    }

    if (source === '@primeuix/themes' || source.startsWith('@primeuix/themes/')) {
        const mapped = source.replace('@primeuix/themes', '@oda-components/theme-tokens');

        return resolveDistPackage(mapped);
    }

    if (source === '@primeuix/style' || source.startsWith('@primeuix/style/')) {
        const mapped = source.replace('@primeuix/style', '@oda-components/style-tokens');
...
function odaComponentsBundleResolver(): Plugin {
    return {
        ...
        resolveId(source) {
            ...
            const legacyPrimeuixMatch = resolveLegacyPrimeuixPackage(source);

            if (legacyPrimeuixMatch) {
                return legacyPrimeuixMatch;
            }
