...
import { getKeyValue, isArray, isEmpty, isFunction, isObject, isString, resolve, toCapitalCase, toFlatCase } from '@app/oda-component/utils/object';
...
import OdaComponentsService from '@app/oda-component/core/service';
import { mergeProps } from 'vue';

const BaseDirective = {
    _getMeta: (...args) => [isObject(args[0]) ? undefined : args[0], resolve(isObject(args[0]) ? args[0] : args[1])],
    _getConfig: (binding, vnode) => (binding?.instance?.$OdaComponents || vnode?.ctx?.appContext?.config?.globalProperties?.$OdaComponents)?.config,
    ...
    _getPTValue: (instance = {}, obj = {}, key = '', params = {}, searchInDefaultPT = true) => {
        ...
        const { mergeSections = true, mergeProps: useMergeProps = false } = instance.binding?.value?.ptOptions || instance.$OdaComponentsConfig?.ptOptions || {};
    ...
    _usePT: (instance = {}, pt, callback, key, params) => {
        ...
        if (pt && Object.hasOwn(pt, '_usept')) {
            const { mergeSections = true, mergeProps: useMergeProps = false } = pt['_usept'] || instance.$OdaComponentsConfig?.ptOptions || {};
    ...
    _extend: (name, options = {}) => {
        const handleHook = (hook, el, binding, vnode, prevVnode) => {
            ...
            el._$instances[name] = {
                ...
                theme: () => el._$instances[name]?.$OdaComponentsConfig?.theme,
        ...
        const handleWatchers = (el) => {
            ...
            // for 'config'
            watchers?.['config']?.call(instance, instance?.$OdaComponentsConfig);
            OdaComponentsService.on('config:change', handleWatchConfig);

            // for 'config.ripple'
            watchers?.['config.ripple']?.call(instance, instance?.$OdaComponentsConfig?.ripple);
            OdaComponentsService.on('config:ripple:change', handleWatchConfigRipple);
        };
        ...
        const stopWatchers = (el) => {
            ...
            if (watchers) {
                OdaComponentsService.off('config:change', watchers.config);
                OdaComponentsService.off('config:ripple:change', watchers['config.ripple']);
