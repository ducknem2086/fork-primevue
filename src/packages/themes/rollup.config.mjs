...
function addThemes() {
    traverseDir(
        ...
        (file, filePath, folderPath) => {
            const inputRoot = path.resolve(__dirname, process.env.INPUT_DIR);
            const folderName = path.relative(inputRoot, folderPath).replaceAll(path.sep, '/');
...
function addLibrary() {
    fs.readdirSync(path.resolve(__dirname, process.env.INPUT_DIR + 'presets'), { withFileTypes: true })
        ...
        .forEach(({ name: folderName }) => {
            ENTRY.format.umd({ name: `OdaComponents.Themes.${folderName[0].toUpperCase() + folderName.slice(1)}`, input: process.env.INPUT_DIR + `presets/${folderName}/index.js`, output: process.env.OUTPUT_DIR + `umd/${folderName}`, minify: true });
        });
}
