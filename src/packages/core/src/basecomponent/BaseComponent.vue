<script>
import { Theme, ThemeService } from '@app/oda-component/styled';
import { findSingle, isElement } from '@app/oda-component/utils/dom';
import { getKeyValue, isArray, isFunction, isNotEmpty, isString, resolve, toFlatCase } from '@app/oda-component/utils/object';
import Base from '@app/oda-component/core/base';
import BaseStyle from '@app/oda-component/core/base/style';
import { useAttrSelector } from '@app/oda-component/core/useattrselector';
...
export default {
    ...
    beforeCreate() {
        ...
        const _useptInConfig = this.$OdaComponentsConfig?.pt?.['_usept'];
        const originalValueInConfig = _useptInConfig ? this.$OdaComponents?.config?.pt?.originalValue : undefined;
        const valueInConfig = _useptInConfig ? this.$OdaComponents?.config?.pt?.value : this.$OdaComponents?.config?.pt;
    ...
    methods: {
        ...
        _getPTValue(obj = {}, key = '', params = {}, searchInDefaultPT = true) {
            ...
            const { mergeSections = true, mergeProps: useMergeProps = false } = this._getPropValue('ptOptions') || this.$OdaComponentsConfig?.ptOptions || {};
        ...
        _usePT(pt, callback, key, params) {
            ...
            if (pt?.hasOwnProperty('_usept')) {
                const { mergeSections = true, mergeProps: useMergeProps = false } = pt['_usept'] || this.$OdaComponentsConfig?.ptOptions || {};
    ...
    computed: {
        globalPT() {
            return this._getPT(this.$OdaComponentsConfig?.pt, undefined, (value) => resolve(value, { instance: this }));
        },
        defaultPT() {
            return this._getPT(this.$OdaComponentsConfig?.pt, undefined, (value) => this._getOptionValue(value, this.$name, { ...this.$params }) || resolve(value, { ...this.$params }));
        },
        isUnstyled() {
            return this.unstyled !== undefined ? this.unstyled : this.$OdaComponentsConfig?.unstyled;
        },
        $id() {
        ...
        $theme() {
            return this.$OdaComponentsConfig?.theme;
        },
        $style() {
        ...
        $styleOptions() {
            return { nonce: this.$OdaComponentsConfig?.csp?.nonce };
        },
        $OdaComponentsConfig() {
            return this.$OdaComponents?.config;
        },
        $name() {
