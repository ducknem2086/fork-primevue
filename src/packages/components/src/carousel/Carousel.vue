...
<script>
import { addClass, find, findSingle, getAttribute, removeClass, setAttribute } from '@app/oda-component/utils/dom';
import { localeComparator, sort } from '@app/oda-component/utils/object';
...
export default {
    ...
    methods: {
        ...
        createStyle() {
            if (!this.carouselStyle) {
                ...
                setAttribute(this.carouselStyle, 'nonce', this.$OdaComponents?.config?.csp?.nonce);
        ...
        ariaSlideNumber(value) {
            return this.$OdaComponents.config.locale.aria ? this.$OdaComponents.config.locale.aria.slideNumber.replace(/{slideNumber}/g, value) : undefined;
        },
        ariaPageLabel(value) {
    ...
    computed: {
        ...
        ariaSlideLabel() {
            return this.$OdaComponents.config.locale.aria ? this.$OdaComponents.config.locale.aria.slide : undefined;
        },
        ariaPrevButtonLabel() {
            return this.$OdaComponents.config.locale.aria ? this.$OdaComponents.config.locale.aria.prevPageLabel : undefined;
        },
        ariaNextButtonLabel() {
            return this.$OdaComponents.config.locale.aria ? this.$OdaComponents.config.locale.aria.nextPageLabel : undefined;
        },
        empty() {
            return !this.value || this.value.length === 0;
        },
        emptyMessageText() {
            return this.$OdaComponents.config?.locale?.emptyMessage || '';
        }
    },
    components: {
        Button,
