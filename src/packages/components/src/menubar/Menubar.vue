    ...
    <div :ref="containerRef" :class="cx('root')" v-bind="ptmi('root')">
        ...
        <slot :id="$id" :name="$slots.button ? 'button' : 'menubutton'" :class="cx('button')" :toggleCallback="(event) => menuButtonClick(event)">
            ...
            <a
                ...
                :aria-controls="$id"
                :aria-label="$OdaComponents.config.locale.aria?.navigation"
...
<script>
import { findSingle, focus, isTouchDevice } from '@app/oda-component/utils/dom';
    ...
    methods: {
        ...
        toggle(event) {
            if (this.mobileActive) {
                ...
                ZIndex.set('menu', this.menubar, this.$OdaComponents.config.zIndex.menu);
                setTimeout(() => {
                    this.show();
                }, 1);
            }
