...
const INLINE_EXTERNALS = ['@oda-components/styled'];
const EXTERNALS = [...GLOBAL_EXTERNALS, ...INLINE_EXTERNALS];

// alias
const ALIAS_ENTRIES = [
    {
        find: /^OdaComponents\/core\/(.*)$/,
...
function addCore() {
    ...
    ENTRY.format.es({ input: process.env.INPUT_DIR + 'config/OdaComponents.js', output: process.env.OUTPUT_DIR + 'config/index' });
    ENTRY.format.es({ input: process.env.INPUT_DIR + 'service/OdaComponentsService.js', output: process.env.OUTPUT_DIR + 'service/index' });
}
