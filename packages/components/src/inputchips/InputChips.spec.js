...
describe('InputChips.vue', () => {
    ...
    it('should add item', async () => {
        ...
        await wrapper.vm.addItem({}, 'oda-components', false);

        await wrapper.setProps({ modelValue: ['oda-components'] });

        expect(addItem).toHaveBeenCalled();
        expect(wrapper.findAll('.p-chip').length).toBe(1);
        expect(wrapper.find('.p-chip-label').exists()).toBe(true);
        expect(wrapper.find('.p-chip-label').text()).toBe('oda-components');
    });
