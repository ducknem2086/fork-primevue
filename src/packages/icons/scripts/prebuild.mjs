...
let modules = {
    ignoredFolders: [],
    esm: [
        `/***************** OdaComponents Icons (Auto-Generated) *****************/
`
    ]
};
            ...
            if (name === folderName || name === `${folderName}icon`) {
                ...
                if (validModuleFolder) {
                    modules.esm.push(
                        `
// ${fileName}
export * from '@oda-components/icons/${folderName}';
export { default as ${fileName} } from '@oda-components/icons/${folderName}';
`
                    );
                }
                ...
                try {
                    fs.readdirSync(folderPath + '/style').forEach((file) => {
                        ...
                        if (subName === `${folderName}style`) {
                            ...
                            if (validModuleFolder) {
                                modules.esm.push(
                                    `export * from '@oda-components/icons/${folderName}/style';
export { default as ${subFileName} } from '@oda-components/icons/${folderName}/style';
`
                                );
                            }
                        }
